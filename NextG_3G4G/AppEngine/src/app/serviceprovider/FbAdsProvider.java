...
//    public static String NATIVE_LARGE_ID_STATIC = "ca-app-pub-3451337100490595/1977608266";
    ...
    public View getNativeAds(final boolean isnativelarge, String adsID, final Activity ctx, final int background_colour, final int header_text_Color, final int footer_text_Color, final int button_baclground_Color, final int button_text_clour) {
       adsID = adsID.trim();
        try {
            ...
            if (isnativelarge)
                return new AdmobNativeAdvanced().getNativeAdvancedAds(ctx, Slave.ADMOB_NATIVE_LARGE_ID_STATIC, true);

            else
                return new AdmobNativeAdvanced().getNativeAdvancedAds(ctx, Slave.ADMOB_NATIVE_MEDIUM_ID_STATIC, false);
        }
    }
    ...
    public void loadAdmobNative(boolean isnativelarge, Context ctx) {
        ...
        if (adMobAds == null)
            adMobAds = new AdMobAds(ctx);
        if (isnativelarge)
            nativeAdContainer.addView(new AdmobNativeAdvanced().getNativeAdvancedAds((Activity) ctx, Slave.ADMOB_NATIVE_LARGE_ID_STATIC, true));
        else
            nativeAdContainer.addView(new AdmobNativeAdvanced().getNativeAdvancedAds((Activity) ctx, Slave.ADMOB_NATIVE_LARGE_ID_STATIC, false));
